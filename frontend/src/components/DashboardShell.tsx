...
export const DashboardShell: React.FC<DashboardShellProps> = ({
  ...
  const sidebarItems: SidebarItem[] = [
    ...
    { id: 'teachers', label: 'Teachers', icon: <Users size={18} />, role: ['ADMIN', 'PRINCIPAL'] },
    { id: 'students', label: 'Students', icon: <GraduationCap size={18} />, role: ['ADMIN', 'PRINCIPAL'] },
    { id: 'classes', label: 'Classes', icon: <BookOpen size={18} />, role: ['ADMIN', 'PRINCIPAL'] },
    { id: 'attendance', label: 'Attendance', icon: <CalendarCheck size={18} />, role: ['ADMIN', 'TEACHER'], requiredFeature: 'ATTENDANCE' },
    { id: 'billing', label: 'Billing & Plans', icon: <CreditCard size={18} />, role: ['ADMIN'], requiredFeature: 'SCHOOL_OPS' },
    { id: 'platform', label: 'SaaS Manager', icon: <Globe size={18} />, role: ['PLATFORM_ADMIN', 'SUPER_ADMIN'] },
  ];

  const hasFeature = (required?: string) => {
    if (!required) return true;
    return featureCodes.includes('*') || featureCodes.includes(required);
  };

  const filteredItems = sidebarItems.filter(item => {
    const roleMatch = !item.role || item.role.includes(userRole);
    const featureMatch = hasFeature(item.requiredFeature);
