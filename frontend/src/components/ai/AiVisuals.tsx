  ...
  <div className="space-y-4 my-6">
    {(items || []).map((item, i) => (
      <div key={i} className="flex gap-4 group">
        <div className="flex flex-col items-center"><div className="w-2.5 h-2.5 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" />{i !== (items?.length || 0) - 1 && <div className="w-0.5 flex-1 bg-white/10 group-hover:bg-emerald-500/30 transition-colors my-1" />}</div>
  ...
  <div className="flex gap-4 overflow-x-auto pb-4 custom-scrollbar">
    {columns.map((col, i) => (
      <div key={i} className="min-w-[200px] flex-1">
        <div className="text-[10px] font-black uppercase tracking-widest text-white/30 mb-3 px-1 flex justify-between">{col.title}<span>{col.items?.length || 0}</span></div>
