  ...
  return (
    <div className="bg-[#0a1829] border border-white/10 rounded-2xl overflow-hidden my-6">
          ...
          <div key={field.name} className="group relative">
            ...
            <div className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${
              ...
              {editing === field.name ? (
                <input 
                  autoFocus
                  className="flex-1 bg-transparent border-none outline-none text-sm text-emerald-50 font-medium"
                ...
                <div className="flex-1 text-sm text-white/70 font-medium truncate">
                  {formData[field.name] || <span className="text-white/20 italic">Empty</span>}
                </div>
              )}
              
              <button 
                onClick={() => setEditing(editing === field.name ? null : field.name)}
                className="p-1.5 rounded-lg hover:bg-white/5 text-white/20 hover:text-emerald-400 transition-colors"
              >
      ...
      <div className="px-5 py-4 bg-emerald-500/5 border-t border-white/5 flex justify-end gap-3">
        <button className="px-4 py-2 rounded-xl text-[11px] font-black uppercase text-white/20 hover:text-rose-400 transition-colors">
          Discard
        </button>
        <button 
          onClick={handleSubmit}
          className="px-5 py-2 rounded-xl bg-emerald-500 text-black text-[11px] font-black uppercase tracking-widest hover:bg-emerald-400 transition-all shadow-[0_0_20px_rgba(16,185,129,0.2)] flex items-center gap-2"
        >
          <Check size={14} />
          {actionLabel}
        </button>
