    ...
    <div className="bg-[#0a1829] border border-white/10 rounded-2xl p-5 my-4 h-[350px]">
      ...
      <ResponsiveContainer width="100%" height="85%">
        <BarChart data={data}>
          ...
          <Tooltip contentStyle={{ backgroundColor: '#020c1b', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px' }} itemStyle={{ color: '#10b981', fontWeight: 800, fontSize: '12px' }} />
          <Bar dataKey="value" fill="#10b981" radius={[6, 6, 0, 0]} fillOpacity={0.8} activeBar={{ fillOpacity: 1, stroke: '#10b981', strokeWidth: 2 }} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
    ...
    <div className="bg-[#0a1829] border border-white/10 rounded-2xl p-5 my-4 h-[350px]">
      ...
      <ResponsiveContainer width="100%" height="85%">
        <PieChart>
          ...
          <Tooltip contentStyle={{ backgroundColor: '#020c1b', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px' }} />
          <Legend verticalAlign="bottom" height={36} iconType="circle" formatter={(value) => <span className="text-[10px] font-bold text-white/40 uppercase">{value}</span>} />
        </PieChart>
      </ResponsiveContainer>
    </div>
  );
