  ...
  const handleSend = async () => {
    ...
    try {
      const response = await schoolOpsApi.visualize({
        ...
        level: 'STANDARD',
        visualizationStyle: 'STEP_LIST',
        premiumRequest: true
      });

      const stepSummary = Array.isArray(response.steps)
        ? response.steps.slice(0, 3).map((step) => `${step.stepNumber}. ${step.heading}: ${step.explanation}`).join('\n')
        : '';
      const answer = [response.summary, stepSummary].filter(Boolean).join('\n\n').trim()
        || "I've analyzed your question. Please try asking with a specific chapter, concept, or problem.";
