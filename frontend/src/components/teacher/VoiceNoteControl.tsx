...
const VoiceNoteControl: React.FC<VoiceNoteControlProps> = ({ onClose, targetUserId, audience = 'PARENTS' }) => {
  ...
  const startRecording = async () => {
    try {
      ...
      if (SpeechRecognition) {
        ...
        recognition.onerror = () => {
          toast.error('Live transcription failed. You can still type the transcript manually.');
        };
  ...
  const handleSend = async () => {
    ...
    if (!transcript.trim()) {
      toast.error('Transcript is required. Record again or type a note before sending.');
      return;
    }
    setLoading(true);
    try {
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              ...
              <textarea
                ...
                rows={5}
                className="w-full bg-transparent text-[#f5efdf] text-sm leading-relaxed outline-none resize-none"
                placeholder="Transcript will appear here. You can edit before sending."
              />
            </div>
