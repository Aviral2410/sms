...
import {
  ...
  Users, BookOpen, Calendar, CreditCard, FileText, Bell, LogOut,
  Sparkles, Menu, X, GraduationCap, Clock, Bus, Library,
  MessageSquare, User, Search, Activity, ArrowLeft,
  TrendingUp, Brain
} from 'lucide-react';
...
function getNavItems(role: string) {
  const all = [
    ...
    // School Admin
    { path: '/dashboard', label: 'Dashboard', icon: LayoutDashboard, roles: ['SCHOOL_ADMIN', 'PRINCIPAL', 'MANAGER'], group: 'Overview', color: '#ffb663' },
    { path: '/students', label: 'Students', icon: Users, roles: ['SCHOOL_ADMIN', 'PRINCIPAL', 'MANAGER', 'TEACHER'], group: 'Academics', color: '#22d3ee' },
    { path: '/teachers', label: 'Teachers', icon: BookOpen, roles: ['SCHOOL_ADMIN', 'PRINCIPAL', 'MANAGER'], group: 'Academics', color: '#a78bfa' },
    { path: '/admissions', label: 'Admissions', icon: FileText, roles: ['SCHOOL_ADMIN', 'PRINCIPAL', 'MANAGER'], group: 'Academics', color: '#a78bfa' },
    { path: '/attendance', label: 'Attendance', icon: Calendar, roles: ['SCHOOL_ADMIN', 'PRINCIPAL', 'MANAGER', 'TEACHER'], group: 'Academics', color: '#34d399' },
    { path: '/exams', label: 'Examinations', icon: GraduationCap, roles: ['SCHOOL_ADMIN', 'PRINCIPAL', 'MANAGER', 'TEACHER'], group: 'Academics', color: '#f472b6' },
    { path: '/timetable', label: 'Timetable', icon: Clock, roles: ['SCHOOL_ADMIN', 'PRINCIPAL', 'MANAGER', 'TEACHER'], group: 'Operations', color: '#818cf8' },
    { path: '/transport', label: 'Transport', icon: Bus, roles: ['SCHOOL_ADMIN', 'PRINCIPAL', 'MANAGER'], group: 'Operations', color: '#fb923c' },
    ...
    { path: '/billing', label: 'Billing & Fees', icon: CreditCard, roles: ['SCHOOL_ADMIN', 'PRINCIPAL', 'MANAGER'], group: 'Finance', color: '#ffb663' },
    ...
    // Teacher
    { path: '/dashboard', label: 'Workspace', icon: Sparkles, roles: ['TEACHER'], group: 'Overview', color: '#a78bfa' },
    { path: '/classes', label: 'My Classes', icon: BookOpen, roles: ['TEACHER'], group: 'Teaching', color: '#22d3ee' },
    { path: '/notices', label: 'Notices', icon: Bell, roles: ['TEACHER'], group: 'Teaching', color: '#fbbf24' },
    { path: '/learn', label: 'AI Visualizer', icon: Brain, roles: ['TEACHER'], group: 'Teaching', color: '#fbbf24' },
    // Student
    { path: '/dashboard', label: 'My Portal', icon: LayoutDashboard, roles: ['STUDENT'], group: 'Overview', color: '#22d3ee' },
    { path: '/attendance', label: 'My Attendance', icon: Calendar, roles: ['STUDENT'], group: 'Academic', color: '#34d399' },
    { path: '/learn', label: 'AI Visualizer', icon: Brain, roles: ['STUDENT'], group: 'Academic', color: '#fbbf24' },
    // Parent
    { path: '/dashboard', label: 'Overview', icon: LayoutDashboard, roles: ['PARENT'], group: 'Overview', color: '#f472b6' },
    { path: '/parents/children', label: 'My Children', icon: Users, roles: ['PARENT'], group: 'Family', color: '#22d3ee' },
    { path: '/parents/messages', label: 'Messages', icon: MessageSquare, roles: ['PARENT'], group: 'Family', color: '#a78bfa' },
    { path: '/learn', label: 'AI Visualizer', icon: Brain, roles: ['PARENT'], group: 'Family', color: '#fbbf24' },
    // Common
    { path: '/profile', label: 'Profile', icon: User, roles: [...PLATFORM_ROLES, 'SCHOOL_ADMIN', 'PRINCIPAL', 'MANAGER', 'TEACHER', 'STUDENT', 'PARENT'], group: 'Account', color: '#64748b' },
  ];
...
export function ModernLayout() {
  ...
  useEffect(() => {
    const loadNotifications = async () => {
      ...
      if (session.role === 'PLATFORM_ADMIN' || session.role === 'SUPER_ADMIN') {
        try {
          const queue = await onboardingApi.listAll();
  ...
  useEffect(() => {
    ...
    const loadSubscriptionTier = async () => {
      try {
        const current = await subscriptionApi.getCurrent(session.tenantId!);
        if (cancelled) return;

        const status = (current.status || '').toUpperCase();
        const planName = (current.planName || '').toUpperCase();
        const isPremium = planName === 'PREMIUM' && ['ACTIVE', 'TRIAL'].includes(status);

        if (session.isPremium !== isPremium) {
          updateSession({ isPremium });
        }
      } catch {
        if (!cancelled && session.isPremium) {
          updateSession({ isPremium: false });
        }
      }
    };
  ...
  }, [session.role, session.tenantId, session.isPremium, updateSession]);
  ...
  const roleLabel = {
    ...
    SCHOOL_ADMIN: 'School Admin', PRINCIPAL: 'Principal', MANAGER: 'Manager',
    TEACHER: 'Teacher', STUDENT: 'Student', PARENT: 'Parent'
  }[session.role || ''] || 'User';

  const roleColor = {
    PLATFORM_ADMIN: '#34d399', SUPER_ADMIN: '#34d399',
    SCHOOL_ADMIN: '#ffb663', PRINCIPAL: '#ffb663', MANAGER: '#ffb663',
    TEACHER: '#a78bfa', STUDENT: '#22d3ee', PARENT: '#f472b6'
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: 'var(--bg-main, #040b14)', fontFamily: "'Manrope','Inter',system-ui,sans-serif", overflow: 'hidden', position: 'relative' }}>
      <AntigravityBackground particleCount={160} />
