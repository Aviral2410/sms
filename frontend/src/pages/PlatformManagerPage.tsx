...
import { request, type SubscriptionPlanResponse } from '../lib/api';
...
export const PlatformManagerPage: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      const data = await request<SubscriptionPlanResponse[]>('/subscriptions/plans');
      setPlans(data as any); // Cast for simplicity since properties match
      
      setSchools([
        { schoolId: '1', schoolName: 'Sunrise Academy', currentPlan: 'PREMIUM', expiryDate: '2026-12-31', status: 'ACTIVE', studentCount: 1240 },
        { schoolId: '2', schoolName: 'Green Valley High', currentPlan: 'BASIC', expiryDate: '2026-06-15', status: 'WARNING', studentCount: 450 },
        { schoolId: '3', schoolName: 'St. Xavier Global', currentPlan: 'FREE', expiryDate: '2026-04-10', status: 'EXPIRED', studentCount: 890 },
      ]);
    } catch (err) {
          ...
          <article className="dashboard-card p-6 border-l-4 border-cyan-500">
            <div className="flex justify-between items-start">
               <div>
                  ...
                  <h3 className="text-2xl font-bold text-white mt-1">2,580 <span className="text-xs text-cyan-400 font-normal">+12%</span></h3>
               </div>
          ...
          <article className="dashboard-card p-6 border-l-4 border-violet-500">
            <div className="flex justify-between items-start">
               <div>
                  ...
                  <h3 className="text-2xl font-bold text-white mt-1">$12.4k <span className="text-xs text-violet-400 font-normal">MRR</span></h3>
               </div>
          ...
          <article className="dashboard-card p-6 border-l-4 border-emerald-500">
            <div className="flex justify-between items-start">
               <div>
                  ...
                  <h3 className="text-2xl font-bold text-white mt-1">42ms <span className="text-xs text-emerald-400 font-normal">EMQX Node</span></h3>
               </div>
          ...
          <article className="dashboard-card wide-card p-0 overflow-hidden">
             <div className="p-6 border-b border-white/5 flex justify-between items-center">
                ...
                <div className="flex gap-4">
                   <span className="flex items-center gap-1 text-[10px] text-slate-500"><HardDrive size={12} /> 12 Active Nodes</span>
                   <span className="flex items-center gap-1 text-[10px] text-slate-500"><Bell size={12} /> 0 Incidents</span>
                </div>
             </div>
