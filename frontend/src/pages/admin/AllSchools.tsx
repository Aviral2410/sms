...
export default function AllSchools() {
  ...
  const handleOpenUpdate = (s: OnboardingResponse, sub?: TenantSubscriptionResponse) => {
    ...
    setUpdateModal({ tenantId, schoolName: s.schoolName, currentPlanId: sub?.planId });
  };
  ...
  const statusColor = (status?: string) => ({
    ACTIVE: '#34d399', TRIAL: '#6366f1', REVOKED: '#f87171', EXPIRED: '#fbbf24'
  }[status || ''] || DIM);
                  ...
                  <motion.tr key={s.onboardingId} whileHover={{ scale: 1.002 }}
                    ...
                    <td style={{ padding: '16px', position: 'relative' }}>
                      <div onMouseEnter={() => setHoverPlan(s.onboardingId)} onMouseLeave={() => setHoverPlan(null)}
                        ...
                        <Zap size={13} fill="#a5b4fc" />{sub?.planName || 'BASIC'}
                  ...
                  <div key={p.planId} onClick={() => setSelectedPlan(p.planId)}
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        ...
                        <div style={{ fontSize: '0.75rem', color: DIM, marginTop: 3 }}>{p.features?.split(',').slice(0, 2).join(' • ')}</div>
                      </div>
