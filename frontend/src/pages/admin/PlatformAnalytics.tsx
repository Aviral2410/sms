      ...
      {loading ? (
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20 }}>
            {[
              { label: 'Active Schools', value: stats.total, icon: Building2, color: '#22d3ee', trend: '+1 this month' },
              { label: 'Platform Capacity', value: `${(subscriptionStats?.totalCapacityStudents || stats.total * 450).toLocaleString()}`, icon: Users, color: '#a78bfa', trend: `${subscriptionStats?.totalActiveSubscriptions || stats.total} active subs` },
              { label: 'Monthly Revenue', value: `$${(subscriptionStats?.monthlyRecurringRevenue || stats.total * 299).toLocaleString()}`, icon: DollarSign, color: '#34d399', trend: 'Real-time billing' },
