...
export default function PlatformDashboard() {
  ...
  const [publicInquiries, setPublicInquiries] = useState<PlatformPublicInquiryResponse[]>([]);
  ...
  useEffect(() => {
    ...
    if (lastMsg?.topic?.startsWith('platform/public-inquiries/')) {
      fetchStats();
    }
  }, [messages]);

  useEffect(() => {
    if ((session.role === 'PLATFORM_ADMIN' || session.role === 'SUPER_ADMIN') && !dashboardWidgets.includes('inquiries')) {
      setDashboardWidgets([...dashboardWidgets, 'inquiries']);
    }
  }, [dashboardWidgets, session.role, setDashboardWidgets]);
  ...
  widgets.inquiries = (
    <div style={{ padding: '24px', borderRadius: 24, background: 'linear-gradient(135deg, rgba(244,114,182,0.1), rgba(34,211,238,0.04))', border: '1px solid rgba(244,114,182,0.18)', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ fontSize: '2.2rem', fontWeight: 900, color: '#fff', marginBottom: 4, letterSpacing: '-0.03em' }}>
        <AnimNum value={publicInquiries.filter((item) => item.status === 'OPEN').length} />
      </div>
      <div style={{ fontSize: '0.82rem', color: '#64748b' }}>Contact and support requests waiting for platform admin action</div>
      {publicInquiries[0] && (
        <div style={{ marginTop: 14, color: '#e2e8f0', fontSize: '0.78rem', lineHeight: 1.5 }}>
          Latest: <strong>{publicInquiries[0].subject}</strong>
        </div>
      )}
    </div>
  );
  ...
  const allWidgetMeta = [
    ...
    { id: 'inquiries', label: 'Public Inbox', icon: Mail, color: '#f472b6' },
  ];
  ...
  const quickActions: { icon: React.ElementType; label: string; desc: string; color: string; path: string | null }[] = [
    ...
    { icon: TrendingUp, label: 'Pricing Control', desc: 'Edit live plan pricing and public features', color: '#fbbf24', path: '/admin/pricing' },
    { icon: LifeBuoy, label: 'Public Inbox', desc: 'Real-time contact and support intake', color: '#f472b6', path: '/admin/inquiries' },
