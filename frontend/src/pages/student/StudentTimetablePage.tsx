...
export default function StudentTimetablePage() {
  ...
  const activeDay = useMemo(() => timetable?.schedule?.[selectedDay], [selectedDay, timetable?.schedule]);
  ...
  return (
    <div className="space-y-6 pb-16">
      ...
      {mode === 'day' && activeDay ? (
        <PortalSection title={`${activeDay.dayOfWeek} periods`} description="Current day detail with subject, teacher, time, and room.">
            ...
            {activeDay.periods.map((period) => (
              <div key={`${activeDay.dayOfWeek}-${period.periodNumber}`} className="glass-panel" style={{ padding: 20, display: 'grid', gap: 10 }}>
                ...
                <div className="grid gap-2" style={{ color: 'var(--text-dim)' }}>
                  ...
                  <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}><MapPin size={14} /> {period.room || 'Room to be announced'}</span>
                </div>
              </div>
            ))}
          </div>
            ...
            {timetable.schedule.map((day) => (
              <div key={day.dayOfWeek} className="glass-panel" style={{ padding: 20, display: 'grid', gap: 14 }}>
                <div className="flex items-center gap-10">
                  ...
                  <div style={{ color: 'var(--text-strong)', fontWeight: 800 }}>{day.dayOfWeek}</div>
                </div>
