...
public class SchoolProvisioningService {
    ...
    private ProvisionSchoolResponse createProvisionedTenant(ProvisionSchoolRequest request) {
        ...
        tenant.setRealmName(request.realmName());
        tenant.setActive(true);
        tenant.setActivatedAt(Instant.now());
        tenant.setRoutingStatus("ACTIVE");
        tenant.setOnboardingStatus("COMPLETED");
        tenantRepository.save(tenant);

        // Initialize default platform subdomain
        if (request.realmName() != null && !request.realmName().isBlank()) {
            TenantDomainEntity domain = new TenantDomainEntity();
            domain.setDomainId(UUID.randomUUID());
            domain.setTenantId(tenantId);
            domain.setDomain(request.realmName());
            domain.setHost(request.realmName()); // Normalized host
            domain.setDomainType("PLATFORM_SUBDOMAIN");
            domain.setIsPrimary(true);
            domain.setIsCanonical(true);
            domain.setIsActive(true);
            domain.setVerificationStatus("VERIFIED");
            domain.setSslStatus("ACTIVE");
            tenantDomainRepository.save(domain);

            TenantRoutingConfigEntity config = new TenantRoutingConfigEntity();
            config.setConfigId(UUID.randomUUID());
            config.setTenantId(tenantId);
            config.setPreferredHost(request.realmName());
            config.setRedirectMode("NONE");
        ...
        // School admin must activate with an activation code; store a non-usable bcrypt value until activation.
        account.setPassword(passwordService.hash(UUID.randomUUID().toString()));
    ...
    public ProvisionUserAccountResponse provisionTenantUser(ProvisionUserAccountRequest request) {
        ...
        account.setPassword(passwordService.hash(request.accessKey()));
