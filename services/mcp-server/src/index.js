...
const enableDockerInsights = (process.env.MCP_ENABLE_DOCKER_INSIGHTS ?? "false").toLowerCase() === "true";
const requireAuth = (process.env.MCP_REQUIRE_AUTH ?? "true").toLowerCase() !== "false";
const docker = enableDockerInsights ? new Docker({ socketPath: "/var/run/docker.sock" }) : null;
...
function requireAuthorizationMiddleware(req, res, next) {
  if (!requireAuth) return next();
  let auth = req.get("authorization") ?? "";

  // EventSource cannot send custom headers; allow passing token as a query param for SSE endpoints.
  // Expected usage: /insights/log-stream?service=...&token=<jwt>
  if (!auth && typeof req.query?.token === "string" && req.query.token.trim()) {
    const raw = req.query.token.trim();
    auth = raw.toLowerCase().startsWith("bearer ") ? raw : `Bearer ${raw}`;
...
async function synthesizeSpeechWithGoogle(text, languageCode) {
  if (!googleTtsApiKey) {
    throw new Error("TTS provider is not configured. Set GOOGLE_TTS_API_KEY.");
  }
  ...
  if (!data.audioContent) {
    throw new Error("TTS provider returned no audio content.");
  }
...
function runWithRequestContext(req, fn) {
  return requestContext.run(
    { 
      ...
      userRole: req.get("x-user-role"),
      schoolId: req.get("x-school-id"),
...
async function getContainerLogs(serviceName) {
  if (!enableDockerInsights || !docker) {
    throw new Error("Docker insights are disabled (set MCP_ENABLE_DOCKER_INSIGHTS=true to enable).");
  }
...
function synthesizeMetrics(data, role) {
  ...
  if (role === "school_admin") {
    ...
    if (dashboard) {
      const attendanceRecords = Number(dashboard.attendanceRecordCount || 0);
      const studentCount = Number(dashboard.studentCount || 0);
      derived.attendanceRate = studentCount > 0
        ? `${((attendanceRecords / studentCount)).toFixed(1)} records/student`
        : "N/A";
      derived.resourceSaturation = dashboard.teacherCount > 0 ? (dashboard.studentCount / dashboard.teacherCount).toFixed(1) : "N/A";
      insights.push(`Attendance recording density is ${derived.attendanceRate}.`);
...
async function answerRoleQuestion({ role, question, schoolId, email }) {
  ...
  const effectiveRole = role || context?.userRole;
...
function makeServer() {
  ...
  server.tool(
    ...
    async ({ subjectId, difficulty, count, context }) => {
      const prompt = [
        "Generate a strict JSON object for a school quiz.",
        "Do not include markdown fences.",
        "Schema: {\"questions\":[{\"id\":number,\"question\":string,\"options\":[string,string,string,string],\"correctAnswer\":string}]}",
        `subjectId=${subjectId}`,
        `difficulty=${difficulty}`,
        `count=${count}`,
        `context=${context || "general syllabus"}`,
        "Ensure each correctAnswer is exactly one of the options."
      ].join("\n");
      ...
      const questions = candidateQuestions
        .map((q, i) => ({
          ...
          options: Array.isArray(q?.options) ? q.options.map((opt) => String(opt)).slice(0, 4) : [],
      ...
      if (!questions.length) {
        throw new Error("Unable to generate valid quiz content from configured LLM provider.");
      }
  ...
  server.tool(
    ...
    async ({ classId, currentTime }) => {
      ...
      const parsedSlots = Array.isArray(timetable)
        ? timetable.map((slot) => {
            const end = String(slot?.endTime || "");
            const start = String(slot?.startTime || "");
            const endParts = end.split(":").map((value) => Number(value));
            const startParts = start.split(":").map((value) => Number(value));
            const endMinutes = endParts.length >= 2 && Number.isFinite(endParts[0]) && Number.isFinite(endParts[1]) ? endParts[0] * 60 + endParts[1] : null;
            const startMinutes = startParts.length >= 2 && Number.isFinite(startParts[0]) && Number.isFinite(startParts[1]) ? startParts[0] * 60 + startParts[1] : null;
            return { raw: slot, endMinutes, startMinutes };
          })
        : [];

      let lastSlot = parsedSlots
        .filter((slot) => slot.endMinutes !== null && slot.endMinutes <= nowMinutes)
        .sort((a, b) => Number(b.endMinutes) - Number(a.endMinutes))[0]?.raw;

      if (!lastSlot) {
        lastSlot = parsedSlots
          .filter((slot) => slot.startMinutes !== null)
          .sort((a, b) => Number(b.startMinutes) - Number(a.startMinutes))[0]?.raw || null;
      }
  ...
  server.tool(
    ...
    async ({ text, language }) => {
      const audioUrl = await synthesizeSpeechWithGoogle(text, language);
      return {
        content: [{ type: "text", text: JSON.stringify({ 
          status: "SUCCESS", 
          text, 
          language, 
          audioUrl,
          message: `TTS conversion completed for ${language}.`
        }, null, 2) }]
      };
    }
  );
...
app.get("/health", (_req, res) => {
  res.json({
    ...
    security: {
      requireAuth,
      enableDockerInsights,
    },
  });
});

// Require auth for all MCP/insights endpoints by default.
app.use("/insights", requireAuthorizationMiddleware);
app.use("/mcp", requireAuthorizationMiddleware);
...
app.get("/insights/container-logs", async (req, res) => {
  try {
    if (!enableDockerInsights) {
      return res.status(403).json({ error: "Docker insights disabled" });
    }
...
app.get("/insights/log-stream", async (req, res) => {
  ...
  try {
    if (!enableDockerInsights || !docker) {
      res.write(`data: ${JSON.stringify({ error: "Docker insights disabled" })}\n\n`);
      return res.end();
    }
    const containers = await docker.listContainers({ all: true });
...
app.listen(port, () => {
  console.info(`sms-mcp-server listening on port ${port}`);
});
