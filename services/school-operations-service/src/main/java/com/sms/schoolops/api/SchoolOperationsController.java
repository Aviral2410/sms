...
public class SchoolOperationsController {
    ...
    public DepartmentResponse updateDepartment(@PathVariable UUID id, @Valid @RequestBody DepartmentUpdate request) {
        return schoolOperationsService.updateDepartment(id, request);
    }

    @DeleteMapping("/departments/{id}")
    public org.springframework.http.ResponseEntity<Void> deleteDepartment(@PathVariable UUID id) {
        schoolOperationsService.deleteDepartment(id);
        return org.springframework.http.ResponseEntity.noContent().build();
    }

    @GetMapping("/departments/hods")
    public List<DepartmentHodView> listDepartmentHods(@RequestParam UUID schoolId) {
        return schoolOperationsService.listDepartmentHods(schoolId);
    }
    ...
    public SubjectResponse updateSubject(@PathVariable UUID id, @Valid @RequestBody SubjectUpdate request) {
        return schoolOperationsService.updateSubject(id, request);
    }

    @DeleteMapping("/subjects/{id}")
    public org.springframework.http.ResponseEntity<Void> deleteSubject(@PathVariable UUID id) {
        schoolOperationsService.deleteSubject(id);
        return org.springframework.http.ResponseEntity.noContent().build();
    }
    ...
    public List<StudentAdmissionResponse> listAdmissions(
            @RequestParam(required = false) UUID schoolId,
            @RequestHeader(value = "X-School-ID", required = false) String schoolIdHeader
    ) {
        return schoolOperationsService.listStudentAdmissions(resolveSchoolId(schoolId, schoolIdHeader));
    }

    @GetMapping("/admissions/next-number")
    public java.util.Map<String, String> getNextAdmissionNumber(
            @RequestParam(required = false) UUID schoolId,
            @RequestHeader(value = "X-School-ID", required = false) String schoolIdHeader
    ) {
        UUID effectiveSchoolId = resolveSchoolId(schoolId, schoolIdHeader);
        return java.util.Map.of("admissionNo", schoolOperationsService.previewNextAdmissionNo(effectiveSchoolId));
    }
    ...
    public SchoolDashboardResponse getDashboard(
        ...
        return schoolOperationsService.getDashboard(resolveSchoolId(schoolId, schoolIdHeader));
    }

    @GetMapping("/teacher-workspace")
    public TeacherWorkspaceResponse getTeacherWorkspace(
            @RequestParam(required = false) UUID schoolId,
            @RequestParam(required = false) String email,
            @RequestHeader(value = "X-School-ID", required = false) String schoolIdHeader,
            @RequestHeader(value = "X-User-Email", required = false) String callerEmailHeader,
            @RequestHeader(value = "X-User-Role", required = false) String callerRoleHeader
    ) {
        UUID effectiveSchoolId = resolveSchoolId(schoolId, schoolIdHeader);
        String effectiveEmail = resolveWorkspaceEmail("TEACHER", email, callerEmailHeader, callerRoleHeader);
        return schoolOperationsService.getTeacherWorkspace(effectiveSchoolId, effectiveEmail);
    }

    @GetMapping("/student-workspace")
    public StudentWorkspaceResponse getStudentWorkspace(
            @RequestParam(required = false) UUID schoolId,
            @RequestParam(required = false) String email,
            @RequestHeader(value = "X-School-ID", required = false) String schoolIdHeader,
            @RequestHeader(value = "X-User-Email", required = false) String callerEmailHeader,
        ...
        UUID effectiveSchoolId = resolveSchoolId(schoolId, schoolIdHeader);
        String effectiveEmail = resolveWorkspaceEmail("STUDENT", email, callerEmailHeader, callerRoleHeader);
        return schoolOperationsService.getStudentWorkspace(effectiveSchoolId, effectiveEmail);
    }

    @GetMapping("/parent-workspace")
    public ParentWorkspaceResponse getParentWorkspace(
            @RequestParam(required = false) UUID schoolId,
            @RequestParam(required = false) String email,
            @RequestHeader(value = "X-School-ID", required = false) String schoolIdHeader,
            @RequestHeader(value = "X-User-Email", required = false) String callerEmailHeader,
        ...
        UUID effectiveSchoolId = resolveSchoolId(schoolId, schoolIdHeader);
        String effectiveEmail = resolveWorkspaceEmail("PARENT", email, callerEmailHeader, callerRoleHeader);
        return schoolOperationsService.getParentWorkspace(effectiveSchoolId, effectiveEmail);
    }

    /**
     * "Me" endpoints that derive identity from gateway-injected headers.
     * These avoid UI-provided schoolId/email parameters for tenant safety.
     */
    @GetMapping("/me/student-workspace")
    public StudentWorkspaceResponse getMyStudentWorkspace(
            @RequestHeader("X-School-ID") String schoolIdHeader,
            @RequestHeader("X-User-Email") String emailHeader
    ) {
        return schoolOperationsService.getStudentWorkspace(resolveSchoolId(null, schoolIdHeader), emailHeader);
    }

    @GetMapping("/me/parent-workspace")
    public ParentWorkspaceResponse getMyParentWorkspace(
            @RequestHeader("X-School-ID") String schoolIdHeader,
            @RequestHeader("X-User-Email") String emailHeader
    ) {
        return schoolOperationsService.getParentWorkspace(resolveSchoolId(null, schoolIdHeader), emailHeader);
    }

    @PostMapping("/users")
    public SchoolUserResponse createUser(
            @Valid @RequestBody SchoolUserRequest request,
            @RequestHeader(value = "X-User-Role", required = false) String roleHeader
    ) {
        return schoolOperationsService.createUser(request, roleHeader);
    }

    @PostMapping("/internal/users/profile")
    public SchoolUserResponse upsertUserProfile(
            @Valid @RequestBody SchoolUserProfileUpsertRequest request,
            @RequestHeader(value = "X-User-Role", required = false) String roleHeader
    ) {
        // Used by auth-service public join flow to keep ids consistent. Treat as internal; lightly gated by role header.
        if (roleHeader == null || roleHeader.isBlank()) {
            throw new IllegalArgumentException("Missing X-User-Role header.");
        }
        String normalized = roleHeader.trim().toUpperCase(java.util.Locale.ROOT);
        if (!normalized.equals("PLATFORM_ADMIN") && !normalized.equals("SUPER_ADMIN") && !normalized.equals("SCHOOL_ADMIN")) {
            throw new IllegalArgumentException("Access denied.");
        }
        return schoolOperationsService.upsertUserProfile(request);
    }
    ...
    public StudentAdmissionResponse getAdmission(@PathVariable UUID admissionId) {
        return schoolOperationsService.getStudentAdmission(admissionId);
    }
    ...
    @GetMapping("/assignments/teacher-subject")
    public List<TeacherSubjectMappingView> listTeacherSubjectMappings(@RequestParam UUID schoolId) {
        return schoolOperationsService.listTeacherSubjectMappings(schoolId);
    }

    @PostMapping("/assignments/class-subject-teacher")
    public ClassSubjectTeacherMappingView assignClassSubjectTeacher(@Valid @RequestBody ClassSubjectTeacherMappingView request) {
        return schoolOperationsService.assignClassSubjectTeacher(request);
    }

    @GetMapping("/assignments/class-subject-teacher")
    public List<ClassSubjectTeacherMappingView> listClassSubjectTeacherMappings(@RequestParam UUID schoolId) {
        return schoolOperationsService.listClassSubjectTeacherMappings(schoolId);
    }

    private UUID resolveSchoolId(UUID schoolIdParam, String schoolIdHeader) {
        UUID headerValue = null;
        if (schoolIdHeader != null && !schoolIdHeader.isBlank()) {
            try {
                headerValue = UUID.fromString(schoolIdHeader.trim());
            } catch (IllegalArgumentException ex) {
                throw new IllegalArgumentException("Invalid X-School-ID header.");
            }
        }

        UUID effective = schoolIdParam != null ? schoolIdParam : headerValue;
        if (effective == null) {
            throw new IllegalArgumentException("schoolId is required.");
        }

        if (schoolIdParam != null && headerValue != null && !schoolIdParam.equals(headerValue)) {
            throw new ForbiddenException("Access denied for the requested school.");
        }
        return effective;
    }

    private String resolveWorkspaceEmail(String workspaceType, String requestedEmail, String callerEmail, String callerRole) {
        String role = callerRole == null ? "" : callerRole.trim().toUpperCase(java.util.Locale.ROOT);

        // Students/parents/teachers can only read their own workspace.
        if (role.equals("STUDENT") || role.equals("PARENT") || role.equals("TEACHER")) {
            if (callerEmail == null || callerEmail.isBlank()) {
                throw new IllegalArgumentException("Missing X-User-Email header.");
            }
            if (requestedEmail != null && !requestedEmail.isBlank()
                    && !callerEmail.equalsIgnoreCase(requestedEmail.trim())) {
                throw new ForbiddenException("You can only access your own " + workspaceType.toLowerCase(java.util.Locale.ROOT) + " workspace.");
            }
            return callerEmail;
        }

        // Admin-style roles may query other accounts (for support), but still require an email.
        if (requestedEmail == null || requestedEmail.isBlank()) {
            if (callerEmail != null && !callerEmail.isBlank()) {
                return callerEmail;
            }
